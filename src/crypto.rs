...
///
/// # Arguments
/// * `public_key_der` — DER-encoded RSAPublicKey (`SEQUENCE { INTEGER n, INTEGER e }`)
///   as expected by ring. Build this from raw `(n, e)` components with
///   [`rsa_components_to_der`].
/// * `message`   — The raw message that was signed (ring hashes internally via SHA-256).
/// * `signature` — Raw RSA signature bytes (same length as the modulus).
///
/// # Errors
/// Returns [`WebAuthnError::SignatureVerificationFailed`] if the signature is
/// invalid, the key is malformed, or the key does not meet ring's 2048-bit minimum.
...
mod tests {
    ...
    // RSA 2048-bit test key: private key (PKCS#8 DER) + public key components.
    // This is a test-only key — never use it outside test code.
    ...
    const TEST_RSA_E: &[u8] = &[0x01, 0x00, 0x01]; // 65537
